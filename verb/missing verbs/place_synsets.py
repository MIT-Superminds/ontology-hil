...
- WordNet hypernym 'Verb Path', ‘Definition’, and ‘Synonyms’ strings
...
- Use the evidence (Verb Path + O*NET tasks) to locate the best-fitting part of the hierarchy.
...
SYNSET: {synset}
VERB PATH: {verb_paths}
Note that this verb path may not be correct, so do not overly depend on it. 
...
EXAMPLE PLACEMENTS:
Synset: adhere.v.01
Verb Path: equal.v.01 > match.v.01 > adhere.v.01
...
Verb Path: treat.v.03 > medicate.v.02 > drug.v.01 > anesthetize.v.01
...
def format_user_prompt(
    ...
    return USER_PROMPT.format(
        synset=synset,
        verb_paths=fmt_block(verb_paths),
        definition=definition,
        synonyms=synonyms,
...
class SynsetPlacer:
    def __init__(self):
        ...
        self.synset_to_vpaths: Dict[str, List[str]] = defaultdict(list)
    ...
    def _index_csv(self):
        ...
        for r in self.rows:
            ...
            if vp:
                self.synset_to_vpaths[syn].append(vp)
        ...
        for syn in list(self.synset_to_tasks.keys()):
            ...
            self.synset_to_vpaths[syn] = unique_preserve_order(self.synset_to_vpaths[syn])
    ...
    def run(self): 
        ...
        for i, syn in enumerate(synsets, 1):
            ...
            user_prompt = format_user_prompt(
                synset=syn,
                verb_paths=self.synset_to_vpaths.get(syn, [])[:20],
                definition=definition,
                synonyms=synonyms,
        ...
        for syn, rec in placements.items():
            ...
            row = {
                ...
                "verb_paths": "|".join(rec.get("evidence", {}).get("verb_paths", [])) if rec.get("status") == "ok" else "",
        ...
        with open(csv_out, "w", newline="", encoding="utf-8") as f:
            ...
            writer = _csv.DictWriter(
                ...
                fieldnames=[
                    ...
                    "new_node_title", "error", "incorrect_explanation", "verb_paths", "sample_onet_tasks", "verb_object_examples", "definition", "synonyms"
                ]
            )
            writer.writeheader()
